...
UniversalActionlibServer::UniversalActionlibServer(string node_name, string action_name)
{
    node_name_ = node_name;
    action_name_ = action_name;
    node_ = rclcpp::Node::make_shared(node_name_);// v ed
    RCLCPP_INFO(rclcpp::get_logger(node_name_),"Creating Node %s...",node_name_.c_str());
}
...
void UniversalActionlibServer::init()
{
    ...
    RCLCPP_INFO(rclcpp::get_logger(node_name_),"Establishing the action server %s...",node_name_.c_str());
    action_server_ = rclcpp_action::create_server<StringTask>(
        node_,
        action_name_,
...
{
    RCLCPP_INFO(rclcpp::get_logger(node_name_),"Setup the rule for accepting a goal.");
    return true;
}

bool UniversalActionlibServer::do_something(string goal, string *feedback, string *result)
{
    RCLCPP_INFO(rclcpp::get_logger(node_name_),"Canceling the goal...\n");
...
{
    RCLCPP_INFO(rclcpp::get_logger(node_name_), "Got request to cancel goal");
    (void)goal_handle;
    return rclcpp_action::CancelResponse::ACCEPT;
}
...
{
    RCLCPP_INFO(rclcpp::get_logger(node_name_), "Executing goal");
    ...
    while(!do_something(tmp_goal, &tmp_feedback, &tmp_result))
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger(node_name_), "Publish Feedback");

        loop_rate.sleep();
    }
    _result->result_s = tmp_result;
    goal_handle->set_succeeded(_result);
    RCLCPP_INFO(rclcpp::get_logger(node_name_), "Goal Suceeded");
}
